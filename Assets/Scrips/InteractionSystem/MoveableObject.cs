...
public class MoveableObject : InteractableObject
{
    ...
    [SerializeField] private float animationSpeed = 100;
    [SerializeField] private bool useAnimationSpeed = false;
    ...
    public void SetToInitialPosition()
    {
        if (transformValuesList.Count > 0)
        {
            transform.SetLocalPositionAndRotation(
                transformValuesList[0].position, 
                Quaternion.Euler(transformValuesList[0].rotation)
            );
            transform.localScale = transformValuesList[0].scale;
        }
    }

    public void SetToAssignIndexPosition()
    {
        if (transformValuesList.Count > 0)
        {
            transform.SetLocalPositionAndRotation(
                transformValuesList[(int)assignIndex].position, 
                Quaternion.Euler(transformValuesList[(int)assignIndex].rotation)
            );
            transform.localScale = transformValuesList[(int)assignIndex].scale;
        }
    }
    ...
    {
        float time = 0f;
        float currentSegementDuration;

        if (useAnimationSpeed)
        {
            currentSegementDuration = Vector3.Distance(transform.localPosition, endTransformValues.position) / animationSpeed;
        }
        else
        {
            ...
            currentSegementDuration = animationDurationPerSegment * distanceTraveled01;
        }
